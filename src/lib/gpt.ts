 ...
 */
export async function askModelForDslMeeting(natural_language_command, user) {
    let prefix = createPromptPrefix(user, natural_language_command);
    let dslOutput = await callGpt3(prefix);
    return dslOutput;
}
 ...
 * @param natural_language_command The command the user input
 * @param user The name of the user. This is ignored and is just present to make the API identical to askModelForDslMeeting. The user is hardcoded as 'Me' in these examples.
...
export function askModelForDslMeetingHardcoded(natural_language_command, user) {
    ...
    console.error(
        'No matching command was found in the set of hardcoded examples. Please confirm that the command text matches exactly.',
    );
    return null;
}
